...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initMenus() throws IOException {
        ...
        try {
            ...
            if (!newMenus.isEmpty()) {
                try{
                    List<Menu> menus = menuRepository.saveAll(newMenus);
                    logger.info("成功导入 " + newMenus.size() + " 个新菜单");
                    // 所有已保存的菜单（含刚刚插入的）
                    List<Menu> all = menuRepository.findAll();
                    // 建立 “component 前缀 -> Menu” 的快速索引
                    Map<String, Menu> prefixMap = all.stream()
                            .collect(Collectors.toMap(
                                    m -> capitalizeFirst(m.getComponent().substring(0, m.getComponent().indexOf("/"))),
                                    m -> m,
                                    (a, b) -> a)); // 如前缀重复可保留第一个
                    // 批量更新 parentId
                    all.forEach(item -> {
                        String prefix = item.getComponent().substring(0, item.getComponent().indexOf("/"));
                        String key    = capitalizeFirst(prefix);
                        Menu parent   = prefixMap.get(key);
