...
onMounted(async () => {
  navigator.modelContext.registerTool({
    ...
    inputSchema: {
      ...
      properties: {
        ...
        permissions: { type: 'array', items: { type: 'number' }, description: '角色拥有的权限' },
      },
  ...
  navigator.modelContext.registerTool({
    ...
    inputSchema: {
      type: 'object',
      properties: {
        role: { type: 'string', description: '需要绑定菜单的角色名称' },
        menu: { type: 'string', description: '需要绑定的菜单名称' },
      },
      required: ['role', 'menu'],
    },
    execute: async ({ role, menu }) => {
